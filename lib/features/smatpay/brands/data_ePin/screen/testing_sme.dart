...
class _TTestingSmeDataScreenState extends State<TTestingSmeDataScreen> {
  ...
  void initState() {
    super.initState();
    balanceController.fetchAccountBalance(); // Fetch balance on screen load
  }

  // Function to fetch the user's account balance from Firestore
  Future<double> fetchAccountBalance(String userId) async {
    try {
      ...
      if (snapshot.exists) {
        ...
        return data['accountBalance'] ?? 0.0; // Fetch account balance
      }
      return 0.0; // Default value if document does not exist
    } catch (e) {
  ...
  // Function to update the user's account balance in Firestore
  ...
  Future<bool> purchaseData() async {
    ...
    if (phone.isNotEmpty) {
      double accountBalance =
          balanceController.balance.value; // Get balance from controller

      // Check if account balance is sufficient
      if (accountBalance >= double.parse(selectedAmount)) {
        // Proceed with the purchase
        bool success =
            await buyData(token, selectedNetwork, phone, selectedDataSize);
        if (success) {
          // Update the balance in Firestore and in the controller
          double newBalance = accountBalance - double.parse(selectedAmount);
          await balanceController.updateBalance(newBalance); // Update balance
          return true;
        }
      } else {
